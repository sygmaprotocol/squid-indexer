...
export const TransfersController = {
  transfers: async function (
    request: FastifyRequest<{ Querystring: ITransfer }>,
    reply: FastifyReply,
  ): Promise<void> {
    try {
      const {
  ...
  transferById: async function (
    request: FastifyRequest<{ Params: ITransferById }>,
    reply: FastifyReply,
  ): Promise<void> {
    const {
      params: { id },
  ...
  transferByTxHash: async function (
    ...
    try {
      const transfer = await transfersService.findTransfersByTxHash(
        txHash,
        domainID,
      );
      void reply.status(200).send(transfer);
  ...
  transfersBySender: async function (
    request: FastifyRequest<{
      Params: ITransferBySender;
      Querystring: ITransfer;
    }>,
    reply: FastifyReply,
  ): Promise<void> {
    const {
    ...
    try {
      const transfers = await transfersService.findTransfersByAccountAddress(
        senderAddress,
        status,
        { page, limit },
      );
  ...
  transfersByResource: async function (
    ...
    try {
      const transfers = await transfersService.findTransfersByResourceID(
        resourceID,
        status,
        { page, limit },
      );
  ...
  transfersBySourceDomainToDestinationDomain: async function (
    request: FastifyRequest<{
      Params: ITransferBySourceDomainToDestinationDomain;
      Querystring: ITransfer;
    }>,
    reply: FastifyReply,
  ): Promise<void> {
    ...
    try {
      const transfers =
        await transfersService.findTransfersBySourceDomainToDestinationDomain(
          sourceDomainID,
          destinationDomainID,
          { page, limit },
        );
      ...
      logger.error(
        "Error occurred when fetching transfers by source and destination domain.",
        e,
      );
      void reply.status(500);
    }
  ...
  transfersByResourceBetweenDomains: async function (
    request: FastifyRequest<{
      Params: ITransferByResourceBetweenDomains;
      Querystring: ITransfer;
    }>,
    reply: FastifyReply,
  ): Promise<void> {
    ...
    try {
      const transfers =
        await transfersService.findTransfersByResourceBetweenDomains(
          resourceID,
          sourceDomainID,
          destinationDomainID,
          { page, limit },
      ...
      logger.error(
        "Error occurred when fetching transfers by resource, source domain and destination domain.",
        e,
      );
      void reply.status(500);
    }
  ...
  transfersByDomain: async function (
    request: FastifyRequest<{
      Params: ITransferByDomain;
      Querystring: ITransferByDomainQuery;
    }>,
    reply: FastifyReply,
  ): Promise<void> {
    const {
    ...
    try {
      const transfers = await transfersService.findTransfersByDomain(
        domainID,
        domain,
        status,
        { page, limit },
